  ...
  await withLoading(async () => {
    try {
      const response = await axios.get("/lapangan/list-pertanyaan", {
        params: { page, size: pageSize.value, type: "QCC" },
      });
      ...
      // Inisialisasi evaluations untuk semua pertanyaan dengan tambahan informasi page
        ...
        subKriteria.pertanyaanList.forEach((pertanyaan) => {
          if (!evaluations.value[pertanyaan.id]) {
            evaluations.value[pertanyaan.id] = {
              ...
              page: pageLapangan.value, // Tambahkan informasi halaman di sini
            };
          }
        });
      });
